...
public function getGroupAdvice($group){
    $return=null;
    $stmt = $this->conn->prepare("SELECT * FROM advice WHERE type=:type");
...
public function mobile($email,$password){
    $return=null;
    $stmt = $this->conn->prepare("SELECT * FROM user WHERE user.phone=:email AND user.password=:password");
    $stmt->bindParam(':email', $email);
    $stmt->bindParam(':password', $password);
    $stmt->execute();
    $stmt->setFetchMode(PDO::FETCH_ASSOC); 
...
public function addProfile($height,$weight,$bmi,$obese,$race,$active,$age,$waist,$gender,$risk,$bmr){
$return =false;
if($this->conn != null) {
$stmt = $this->conn->prepare("INSERT INTO 
	history(height,weight,bmi,obese,race,physical_activity,age,waist_measurement,gender,risks,bmr) 
	VALUES 
	(:height,:weight,:bmi,:obese,:race,:physical_activity,:age,:waist_measurement,:gender,:risks,:bmr)");
